...
void get_age(unsigned object_id, unsigned chunk_id, unsigned *age)
{
	...
	index = (chunk_id & 0xff) | (object_id << 8);
	if (index >= 4194304)
		printk("unusable index ,locate in %d", __LINE__);
...
{
	u32 i;
	if (uft_discretizer.group_count == 0)
		return;
...
unsigned discretize_uft_value(s64 uft_value)
{
		...
		uft_discretizer.groups[best_group].min_val > 0)
	{
		...
		/* 检查是否违反 max < 2*min 条件 */
		int violates_condition = (group_max >= 5 * group_min / 4) ? 1 : 0;
...
struct QTableEntry *initializeQTable(u64 capacity)
{ // 动态数组在堆区分配
...
{
	int reward;
	if (num_gc_copies == 0 || deleted_pages == 0)
		reward = 128;
	else
		reward = num_gc_copies < deleted_pages ? (10 * deleted_pages / num_gc_copies) / 10 : (10 * num_gc_copies / deleted_pages) / 10;
							   ...
							   enum Action action)
{
	...
	{ // 达到int最大值，该state下q值均大于0，减去最小值确保相对顺序
		...
		for (i = 0; i < 4; i++)
		{
			if (i == 0)
				...
				temp = ACTION_THREE;
			struct QTableEntry *temp_QE = findOrInsertQEntry(table, state, temp);
			if (temp_QE && temp_QE->qValue < minqvalue)
			{ /* 添加NULL检查 */
...
static int yaffs_find_alloc_block(struct yaffs_dev *dev, int hot)
{
	...
	else if (2 == hot) // any emtpy is ok for this kind of data
	{
		...
		for (i = dev->internal_start_block; i <= dev->internal_end_block; i++)
		{
			...
			if (dev->alloc_block_finder2 < dev->internal_start_block || dev->alloc_block_finder2 > dev->internal_end_block)
			{
				dev->alloc_block_finder2 = dev->internal_start_block;
			}
	...
	else if (3 == hot) /* Cold data, write to  block with max erasure count. */
	{
		...
		for (i = dev->internal_start_block; i <= dev->internal_end_block; i++)
		{
			...
			if (dev->alloc_block_finder3 < dev->internal_start_block || dev->alloc_block_finder3 > dev->internal_end_block)
			{
				dev->alloc_block_finder3 = dev->internal_start_block;
			}
	...
	else /* Cold data, write to  block with max erasure count. */
	{
		...
		for (i = dev->internal_start_block; i <= dev->internal_end_block; i++)
		{
			...
			if (dev->alloc_block_finder4 < dev->internal_start_block || dev->alloc_block_finder4 > dev->internal_end_block)
			{
				dev->alloc_block_finder4 = dev->internal_start_block;
			}
							 ...
							 struct yaffs_block_info **block_ptr, int hot)
{
	...
	else if (2 == hot) // now use block3
	{
		...
		if (dev->n_erased_blocks < dev->param.n_reserved_blocks && dev->alloc_page2 == 0)
			yaffs_trace(YAFFS_TRACE_ALLOCATE, "Allocating reserve");
	...
	else if (3 == hot)
	{
		...
		if (dev->n_erased_blocks < dev->param.n_reserved_blocks && dev->alloc_page3 == 0)
			yaffs_trace(YAFFS_TRACE_ALLOCATE, "Allocating reserve");
	...
	else
	{
		...
		if (dev->n_erased_blocks < dev->param.n_reserved_blocks && dev->alloc_page4 == 0)
			yaffs_trace(YAFFS_TRACE_ALLOCATE, "Allocating reserve");
...
// add by ry 2024.05.10
		...
		if (current_state == NULL)
			printk("error with get_usable_state in %d", __LINE__);
		if (new_start)
		{ // 第一次垃圾回收的初始状态
			new_start = 0;
		}
								 ...
								 struct yaffs_ext_tags *tags, int use_reserver)
{
	...
	do
	{
		...
		*/
		...
		//     time = now;
		// }
		if (tags->obj_id > max_objid)
			max_objid = tags->obj_id;
		chunk = yaffs_alloc_chunk(dev, use_reserver, &bi, hot);
		if (chunk > max_chunkid)
			max_chunkid = chunk;
		// set_time_hot(tags->obj_id,tags->chunk_id, now, ++cnt);
		set_age(tags->obj_id, tags->chunk_id, ++PSN);
...
static void yaffs_retire_block(struct yaffs_dev *dev, int flash_block)
{
	...
	if (yaffs_mark_bad(dev, flash_block) != YAFFS_OK)
	{
		...
		else
		{
			...
			if (dev->tagger.write_chunk_tags_fn(dev, chunk_id - dev->chunk_offset,
												buffer,
												&tags) != YAFFS_OK)
				yaffs_trace(YAFFS_TRACE_ALWAYS,
										 ...
										 int old_chunk, u8 *buffer, int aver_uft, int start_new)
{
	...
	if (object && !yaffs_skip_verification(dev))
	{
		if (tags.chunk_id == 0)
			...
			matching_chunk =
				yaffs_find_chunk_in_file(object, tags.chunk_id,
										 NULL);
		...
		else
		{
			...
			for (i = 0; i < BLOCK_NUMBER; i++)
			{
				...
				if (bi->block_state == YAFFS_BLOCK_STATE_FULL)
				{
					...
					cwps = sum_block[dev->gc_block_finder - 1].sum + sum_block[dev->gc_block_finder - 1].cnt * (now - sum_block[dev->gc_block_finder - 1].chunk_seq);
					if (cwps < sum_block[dev->gc_block_finder - 1].sum)
					{
						printk("overflow happen here %d \r\n", __LINE__);
					}
				}
				cwps = cwps * (dev->param.chunks_per_block - pages_used) / (dev->param.chunks_per_block + pages_used); // fcy changed
				if (cwps > max_cwps)
				{
					max_cwps = cwps;
					dev->gc_dirtiest = dev->gc_block_finder;
...
// add by ry to calculate reward according to number of gc_copies
			...
			if (Qlearning_count)
			{
				...
				if (gc_ok == YAFFS_OK)
					ql_stats.gc_success_count++;
				else
					ql_stats.gc_fail_count++;
#endif
...
/* ========== 详细的调试打印 ========== */
	...
	if (Qlearning_count < num_trainings && Qlearning_count % 1000 == 0 && Qlearning_count > 0)
	{
		...
		if (ql_stats.gc_operations > 0)
		{
			...
			u32 recovery_rate = ql_stats.gc_success_count > 0 ? (100 * ql_stats.chunks_recovered_total) / (ql_stats.gc_operations * dev->param.chunks_per_block) : 0;
...
int yaffs_guts_initialise(struct yaffs_dev *dev)
{
	...
	if (!QTable)
		printk("failed to alloc space for QTable");
	else
		printk("succeed to initialize QTable");
#endif
	if (yaffs_guts_ll_init(dev) != YAFFS_OK)
		return YAFFS_FAIL;
