	...
	if (n_written > 0) {
		...
		*pos = ipos;
		if (ipos > inode->i_size) {
			inode->i_size = ipos;
			inode->i_blocks = (ipos + BLOCK_NUMBER-1) >> 12;
	...
	if (error == 0) {
		...
		if (!error) {
			...
			if (attr->ia_valid & ATTR_SIZE) {
				...
				inode->i_blocks = (inode->i_size + BLOCK_NUMBER-1) >> 12;
			}
		}
	...
	while (context->bg_running) {
		...
		if (time_after(now, next_gc) && yaffs_bg_enable) {
			if (!dev->is_checkpointed) {
				...
				//add by ry 2024 .05.04 to solve mcsgc problem
				while(!gc_result && max_tries>0)
				{
					max_tries--;
				gc_result = yaffs_bg_gc(dev, urgency);
				}
				if(!gc_result) printk("can not reclaim enough space after 10 tries");
				//add end
				if (urgency > 1)
					next_gc = now + HZ / 20 + 1;
				else if (urgency > 0)
...
#ifdef YAFFS_GC_ED_MODIFIED	/* Added by Ed. 2014.05.01 */
	options->no_cache = 1;
	options->skip_checkpoint_read = 1;
	options->skip_checkpoint_write = 1;
	options->empty_lost_and_found = 0;
...
static void yaffs_dump_dev_part1(struct seq_file *m, struct yaffs_dev *dev)
{
	...
	static int bs[BLOCK_NUMBER+2],i;  
//	static int count;
	extern void yaffs_count_blocks_by_erasures(struct yaffs_dev *dev, int *bs);
	extern long int n_blk_sd[];
	extern int n_blk_erasure_max;
	extern int n_blk_erasure_min;
    extern unsigned chunk_seq;
...
#endif
#if 1
    seq_printf(m,"\r\nthe erase counts of all blocks are:\n");
	for(i=0;i<BLOCK_NUMBER;i++)
	{
	   seq_printf(m,"%d ", n_blk_erasures[i]);
	}
#endif

   //seq_printf(m,"\r\nthe experiment takes %d seconds\n",(jiffies - start_time)/HZ);
    seq_printf(m,"\r\nthe final chunk_seq is %d\n",chunk_seq);
	seq_printf(m, "\nn_blk_erase_min.... %d-%d\n", bs[BLOCK_NUMBER],n_blk_erasure_min);
	seq_printf(m, "n_blk_erase_max.... %d-%d\n", bs[BLOCK_NUMBER+1],n_blk_erasure_max);
	seq_printf(m,"dev->alloc_block = %d ",dev->alloc_block);
#if defined(GC_Qlearning)
	seq_printf(m,"biggest object id = %d ",max_objid);
	seq_printf(m,"biggest chunk id = %d ",max_chunkid);
	seq_printf(m,"dev->alloc_block2 = %d ",dev->alloc_block2);
	seq_printf(m,"dev->alloc_block3 = %d ",dev->alloc_block3);
	seq_printf(m,"dev->alloc_block4 = %d\n",dev->alloc_block4);
#endif	
	seq_printf(m,"n_blk_erase_sd...... ");
	
	for(i=0; i<dev->n_erasures/500;i++)//for(i=0; i<min(dev->n_erasures/500,1023);i++) modified by ed 2024.04.17
		seq_printf(m,"%d ",n_blk_sd[i]);
...
//	static int count;
    ...
    extern unsigned chunk_seq;
	extern unsigned start_time;
	extern int n_blk_erasures[BLOCK_NUMBER]; //fcy add
#if 1
    buf += sprintf(buf,"\r\nthe erase counts of all blocks are:\n");
	for(i=0;i<BLOCK_NUMBER;i++)
	{
	   buf += sprintf(buf,"%d ", n_blk_erasures[i]);
	}
#endif
   ...
   // buf += sprintf(buf,"\r\nthe experiment takes %d seconds\n",(jiffies - start_time)/HZ);
    buf += sprintf(buf,"\r\nthe final chunk_seq is %d\n",chunk_seq);
	buf += sprintf(buf, "\nn_blk_erase_min.... %d-%d\n", bs[BLOCK_NUMBER],n_blk_erasure_min);
	buf += sprintf(buf, "n_blk_erase_max.... %d-%d\n", bs[BLOCK_NUMBER+1],n_blk_erasure_max);
	buf += sprintf(buf,"dev->alloc_block = %d ",dev->alloc_block);
#if defined(GC_Qlearning)
	buf += sprintf(buf,"dev->alloc_block2 = %d ",dev->alloc_block2);
		buf += sprintf(buf,"dev->alloc_block3 = %d ",dev->alloc_block3);
		buf += sprintf(buf,"dev->alloc_block4 = %d\n",dev->alloc_block4);
#endif
	buf += sprintf(buf,"n_blk_erase_sd...... ");
	
	for(i=0; i<dev->n_erasures/500;i++)
		buf += sprintf(buf,"%d ",n_blk_sd[i]);
