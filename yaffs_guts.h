 ...
 * a maximum file size of around BLOCK_NUMBERGbytees with 2k chunks.
 */
...
#define YAFFS_BYTES_PER_CHUNK		BLOCK_NUMBER
...
/* Modified by Ed. 2014.05.01 */ // add 2024.4.14
#define YAFFS_GC_ED_MODIFIED

#ifdef YAFFS_GC_ED_MODIFIED
    #define BLOCK_NUMBER 4096 //add by ed 2024.4.14
	#define GC_Qlearning//关闭最冷块回收
#endif

#if defined(GC_Qlearning )
#define Tmin      (10)
#define	INIT_TWL  (100)	/* Twl */
#define Hmax      (16)
#define NUM_C     (1)

struct fagc_hdt_item
{
	unsigned  time;		/* update time */
	unsigned  cnt;	
};
struct fagc_page_item
{
	unsigned  object_id;	/* file object id */
	unsigned  chunk_id;	/* file chunk id */
};

struct fagc_cps_item
{
    unsigned sum;
	unsigned cnt;
	unsigned chunk_seq;
...
//4096项 是否可以压缩
//空闲块被分配为allocting_block的时刻，用以引入时间因子
struct block_alloc_time{
	unsigned long time;
};

  
//Q-table 的条目，存储每个状态-动作对的Q值  
/*
要么以类（四类）储存对应的状态动作对，要么每条条目加一个该条目下state与action对应迁移进的块id
*/
struct QTableEntry{  
    int qValue;
	struct State* state;  
	enum Action action;
	// unsigned block;//储存对应的迁移进块号 chunk_id % dev->param.chunks_per_block
} ;  
...
struct yaffs_param {
	...
	u32 total_bytes_per_chunk;	/* Should be >= BLOCK_NUMBER, does not need to
					 be a power of 2 */
...
struct yaffs_dev {
	...
	#if defined(GC_Qlearning)
	int alloc_block2;			/* for cold */
	u32 alloc_page2;
	int alloc_block_finder2;	/* for cold */

	int alloc_block3;			
	u32 alloc_page3;
	int alloc_block_finder3;

	int alloc_block4;		
	u32 alloc_page4;
	int alloc_block_finder4;
	#endif
